...
async def data_request_routing(sio, cmd, data, logger, sid):
    ...
    async with AsyncSessionLocal() as dbsession:
        ...
        if cmd == "get-tle-sources":
            ...
            preferences = await crud.preferences.fetch_preference_for_userid(dbsession, user_id=None)
...
async def data_submission_routing(sio, cmd, data, logger, sid):
    ...
    async with AsyncSessionLocal() as dbsession:
        ...
        if cmd == "submit-tle-sources":
            ...
            add_reply = await crud.sers.add_user(dbsession, data)
            ...
            locations = await crud.locations.fetch_location_for_userid(dbsession, user_id=data['userid'])
